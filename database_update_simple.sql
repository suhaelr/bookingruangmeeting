...
-- Database Update - Simple Version (No Dynamic SQL)
-- =============================================

-- 1. CREATE MEETING ROOMS TABLE (if not exists)
-- =============================================
...
-- 3. ADD COLUMNS TO USERS TABLE (run these one by one if needed)
-- =============================================
...
ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `full_name` varchar(255) AFTER `username`;

-- Add phone column
ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `phone` varchar(255) NULL AFTER `full_name`;

-- Add department column
ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `department` varchar(255) NULL AFTER `phone`;

-- Add role column
ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `role` enum('admin','user') NOT NULL DEFAULT 'user' AFTER `department`;

-- Add avatar column
ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `avatar` varchar(255) NULL AFTER `role`;

-- Add last_login_at column
ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `last_login_at` timestamp NULL AFTER `avatar`;

-- 4. INSERT SAMPLE DATA - USERS (only if not exists)
-- =============================================
INSERT IGNORE INTO `users` (`username`, `name`, `email`, `password`, `full_name`, `phone`, `department`, `role`, `avatar`, `last_login_at`, `email_verified_at`, `created_at`, `updated_at`) VALUES
...
-- 5. INSERT SAMPLE DATA - MEETING ROOMS (only if not exists)
-- =============================================
INSERT IGNORE INTO `meeting_rooms` (`name`, `description`, `capacity`, `amenities`, `location`, `hourly_rate`, `images`, `is_active`, `created_at`, `updated_at`) VALUES
...
-- 6. INSERT SAMPLE DATA - BOOKINGS (only if not exists)
-- =============================================
INSERT IGNORE INTO `bookings` (`user_id`, `meeting_room_id`, `title`, `description`, `start_time`, `end_time`, `status`, `attendees_count`, `attendees`, `attachments`, `special_requirements`, `total_cost`, `created_at`, `updated_at`) VALUES
...
-- NOTES:
-- 1. This SQL uses simple ALTER TABLE with IF NOT EXISTS
-- 2. Tables that already exist will be skipped
-- 3. Data that already exists will be skipped
-- 4. Only missing columns will be added to existing tables
-- 5. Password untuk semua user adalah: 'password' (sudah di-hash)
-- 6. Admin login: username='admin', password='password'
