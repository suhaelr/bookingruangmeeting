            ...
            <div class="flex justify-between items-center h-16">
                <div class="flex items-center">
                    ...
                    <div class="flex-shrink-0">
                        <i data-feather="calendar" class="text-2xl text-black" style="width: 24px; height: 24px;"></i>
                    </div>
                </div>
                    ...
                    <div class="relative">
                        <button id="avatarDropdownBtn" 
                            ...
                            @if(session('user_data.avatar'))
                                <img src="{{ asset('storage/' . session('user_data.avatar')) }}" 
                                     alt="{{ session('user_data.full_name') ?? 'User' }}"
                        ...
                        <div id="avatarDropdown" 
                            ...
                            <div class="px-4 py-3 border-b border-gray-200">
                                <p class="text-sm font-semibold text-gray-900">{{ session('user_data.full_name') ?? 'User' }}</p>
                                <p class="text-xs text-gray-500 truncate">{{ session('user_data.email') ?? 'N/A' }}</p>
                            </div>
                            <a href="{{ route('user.profile') }}" 
                               class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-700 transition-colors duration-150 flex items-center">
